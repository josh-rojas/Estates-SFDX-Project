...
jest.mock(
  ...
  () => {
    const { updateRecord: actualUpdateRecord } = jest.requireActual(
      "lightning/uiRecordApi"
    );
    return {
...
describe("c-record-pathway-selection", () => {
  ...
  it("updates Case with Final Grant pathway when button clicked", async () => {
    ...
    const finalGrantButton = Array.from(buttons).find(
      (btn) => btn.label === "Final Grant"
    );
  ...
  it("updates Case with New DAF Account pathway when button clicked", async () => {
    ...
    const newDAFButton = Array.from(buttons).find(
      (btn) => btn.label === "New DAF Account"
    );
    newDAFButton.click();
  ...
  it("updates Case with Disclaim Assets pathway when button clicked", async () => {
    ...
    const disclaimButton = Array.from(buttons).find(
      (btn) => btn.label === "Disclaim Assets"
    );
